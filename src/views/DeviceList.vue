  ...
  <div class="deviceList">
    <div class="module">
      ...
      <el-form :inline="true"  v-if="queryType==='simple'">
        <el-form-item>
          <el-input style="width:300px" prefix-icon="el-icon-search" @keyup.enter.native="doRefresh('search')"
                    v-model="searchKey" placeholder="请输入设备名称或者设备编号" size="medium"
                    clearable></el-input>
        </el-form-item>
        <el-form-item style="float: right">
          <el-button type="primary" style="background-color:#2c2b30;border:none " @click="doRefresh('search')" size="medium" icon="el-icon-search">查询</el-button>
          <el-button @click="doReset()" size="medium" icon="el-icon-refresh" class="btnChangeColor">重置</el-button>
          <el-button type="text" style="color: #2c2b30" icon="el-icon-arrow-down" @click="doSwitchQueryType()">高级查询</el-button>
        </el-form-item>
      </el-form>
      <el-form  label-width="100px" v-if="queryType==='advanced'">
        ...
        <el-row :gutter="24" >
          ...
          <el-col :span="5" :offset="0" align="right" style="float: right;" >
            <el-button type="primary" style="background-color:#2c2b30;border:none " @click="doRefresh()" size="medium" icon="el-icon-search">查询</el-button>
            <el-button @click="doReset()" size="medium" icon="el-icon-refresh">重置</el-button>
            <el-button type="text" style="color: #2c2b30" icon="el-icon-arrow-up" @click="doSwitchQueryType()">高级查询</el-button>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  methods: {
     ...
     * */
    initSearchData(){
      this.$api.getSelectList(this.equipmentNo,this.name,this.plateNo,this.types,1, 9999).then(res => {
        if(res.data.code===200){
          ...
          for(let info of data){
            ...
            if(!permissionKey.includes(permission)){
              Vue.set(this.permissionList,permission,info.permissionTypeLabel)
            }
          }
        }
      })
    ...
    initDeviceList(pageNo,pageSize) {
      this.loading=true
      this.$api.getSelectList(this.equipmentNo,this.name,this.plateNo,this.types,pageNo, pageSize).then(res => {
        if(res.data.code===200){
