      ...
      <div class="module">
        <el-row :gutter="20">
          <el-col :span="2" :offset="2" style="margin-right: -20px">
            <el-select v-model="deviceMessage" placeholder="请选择" size="medium" @change="deviceMessageChange">
              <el-option v-for="item in deviceMessageOptions" :key="item.value" :label="item.label" :value="item.value">
              </el-option>
            </el-select>
          </el-col>
          <el-col :span="4">
            <el-select v-model="deviceInfo" multiple collapse-tags filterable placeholder="请选择" size="medium" style="width:300px">
              <el-option v-for="(item,index) in deviceInfoOptions" :key="index" :label="item" :value="item">
              </el-option>
            </el-select>
          </el-col>
          ...
          <el-col :span="5" >
              <el-date-picker v-model="chooseTime" type="datetimerange" :editable="false" clearable size="medium" range-separator="-" style="width:280px"
                              start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickerOptions" align="center" :format="'yyyy-MM-dd'">
              </el-date-picker>
          </el-col>
          <el-col :span="5">
            <el-col :span="7">
              <el-button type="primary" style="background-color:#2c2b30;border:none " size="medium" icon="el-icon-search" @click="foundInfByCondition">查询</el-button>
            </el-col>
            <el-col :span="7">
              <el-button type="primary" style="background-color:#2c2b30;border:none " size="medium" icon="el-icon-download" @click="downloadData">导出</el-button>
            </el-col>
          </el-col>
        </el-row>
        <div class="content">
          <div class="countData">
            <div>
              <p>行驶里程</p>
              <h4><span>{{roadHaul}}</span> 公里</h4>
            </div>
            <div>
              <p>行驶油耗</p>
              <h4><span>{{travelOil}}</span> 升</h4>
            </div>
            <div>
              <p>工作时长</p>
              <h4><span>{{workHours}}</span> 小时</h4>
            </div>
            <div>
              <p>工作油耗</p>
              <h4><span>{{workOils}}</span> 升</h4>
            </div>
            <div>
              <p>吊载次数</p>
              <h4><span>{{craneLoadCount}}</span> 次</h4>
            </div>
          </div>
          <div class="module">
            <el-table
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  <span>
                    <el-button @click="deviceDetailInfo(scope.row.deviceId)" type="text" size="medium">详情</el-button>
                    <el-button @click="sevenDayWorkConditionInfo(scope.row.deviceId)" type="text" size="medium">七日工况分析</el-button>
                  </span>
                </template>
              </el-table-column>
            </el-table>
...
export default {
  data() {
    return {
      ...
      plateNoList:[], //车牌信息
      deviceMessageOptions: [{ //第一个搜索的条件类型
        value: 'equipmentNo',
        label: '设备编码'
      }, {
        value: 'equipmentName',
      ...
      chooseTime: [new Date().getTime()-3600 * 1000 * 24,new Date().getTime()-3600 * 1000 * 24], //时间范围
      pickerOptions: {
        shortcuts: [{
          text: '最近一周',
          onClick(picker) {
  ...
  methods: {
      ...
      if(this.deviceInfo.length>0){
        if(this.deviceMessage==='equipmentName'){
          for(let name of this.deviceInfo){
            this.allDeviceInfo.filter(p => p.entity == null && p.name != null && name===p.name).map(p => {
              equipment.push(p.equipmentNo)
            });
          }
        }else if(this.deviceMessage==='plateNo'){
          for(let name of this.deviceInfo){
            this.allDeviceInfo.filter(p => p.entity == null && p.plateNo != null && name===p.plateNo).map(p => {
              equipment.push(p.equipmentNo)
            });
          }
        }else{
    ...
    async workOtherInfo(equipment,pageNo){
      ...
      //设备行程，油况等基础信息
      let detailInfo=await this.workDetailInfo(equipment,startDate,endDate,pageNo)
      ...
      if(detailInfo.data.code===200){
        ...
        for(let deviceNo of businessNo){
          this.$api.getHistoryTrackDetail(startDate+' 00:00:00',endDate+' 23:59:59',deviceNo).then(res=>{
            if(res.data.code===200){
              ...
              if(locationInfo.length>0){
                for(let info of this.tableData){
                  if(info.deviceId===locationInfo[0].equip_code){
                    ...
                    Vue.set(info,'lastPositioningTime',locationInfo[0].time)
                    break;
                  }
                }
              }
      ...
      this.$api.getEquipmentBusinessNameByNo(equipment).then(res=>{
        if(res.data.code===200){
          ...
          for(let info of data){
            ...
            this.tableData.filter(p => p.entity == null && p.deviceId != null).map(p => {
    ...
    workStatisticalInfo(equipment,startDate,endDate){
      this.$api.getWorkStatInfo(equipment,startDate,endDate).then(res=>{
        if(res.data.code===200){
          let data=res.data.data
    ...
    workDetailInfo(equipment,startDate,endDate,pageNo){
      return this.$api.getWorkDetailInfoByEquipmentNo(equipment,startDate,endDate,pageNo,this.pageSize);
    },
    /**
     * 设备信息类型切换
     * */
    ...
    downloadData(){
      ...
      let startDate=dateFormat(this.chooseTime[0],'yyyy-MM-dd')
      let endDate=dateFormat(this.chooseTime[1],'yyyy-MM-dd')
      this.$api.drivingDataDownload(equipment,startDate,endDate).then(res=>{
        let fileNameUnicode = res.headers["content-disposition"].split(
                "filename="
        )[1];
